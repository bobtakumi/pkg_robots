...
- [ ] **R4 rules proposer**: lint が検出する昇格候補（現在22件）・タグ違反・孤立を「提案」として proposals.jsonl に流し込み、週次シートの種目に加える。
  ...
  体裁の確定仕様（2026-08-03 ユーザー指示）: シートの提案本文は1行に主張・引用・帰結を詰め込まず、
  **親の箇条書き=ノート自身の主張・タブ1つ下げた子=文献の引用[[リンク]]や帰結**に構造化する。
  チェックボックスは1行1個。実例は vault `_Reports/review-bundles/2026-08-02_permanent-links/views/バッチ2-…` の提案1-1。
  将来 sheet.py が複数行の提案を生成する種目（draft/merge/spawn）ではこの形を必須とする。
